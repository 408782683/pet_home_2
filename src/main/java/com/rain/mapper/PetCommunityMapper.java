...
public class PetCommunityMapper {
    ...
    public List<Hospital> findHospitalByPage(String name, String type, String level, Integer page, Integer size) throws SQLException {
        ...
        try (Connection conn = JdbcUtil.getConnection(); PreparedStatement ps = conn.prepareStatement(sql.toString())) {
            ...
            while (rs.next()) { Hospital h = new Hospital(); h.setId(rs.getInt("id")); h.setName(rs.getString("name")); h.setAddress(rs.getString("address")); h.setPhone(rs.getString("phone")); h.setType(rs.getString("type")); h.setLevel(rs.getString("level")); h.setDescription(rs.getString("description")); h.setImages(rs.getString("images")); h.setCreateTime(rs.getTimestamp("create_time")); list.add(h); }
            return list;
        }
    }

    public Integer countHospital(String name, String type, String level) throws SQLException {
